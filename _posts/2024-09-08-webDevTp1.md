...
Le symbole > indique que la règle concerne une relation parent-enfant **directe**. Cela signifie que le style ne sera appliqué qu'à la balise <ul> qui se trouve directement sous la balise <nav>. Par contre, l'absence de > dans la règle nav>ul>li a veut dire que le style est appliqué sur a et elle n'est pas en lien de parenté direct avec li.
...
l'ajouter via un lien dans les pages concernées. Ajoutez ce code au niveau de la balise head et copiez tout le contenu de la balise style dans le nouveau fichier
style.css.
...
<div class="alert alert-remark">
  ...
  <div class="alert-body-remark">
        Vous allez créer deux nouvelles pages HTML. Une qui s'appelle `cheesecakes.html`, la deuxième s'appelle `pies.html` Le but est de consacrer une
    page à part entière pour chaque type de gâteaux dans la pâtisserie. Finalisez sa mise en forme.
   </div>
</div>
---
