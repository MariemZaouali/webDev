...
```
Passons Dans le fichier `quotelist.js`, insérez le code suivant:
...
Nous ajoutons maintenant un **Event Handler** dans la page `index.html`, le code commence par ajouter un appeal à ce handler dans la balise body (ce code doit
être copié avant la balise footer)
```html
...
```
Jusqu'à ce niveau, rien ne fonctionne ! Il faut mettre au point votre `index.js`
```
...
Avant de passer au lancement de notre site, vérifiez le contenu de la section scropt du fichier `package.json`de manière à avoir:
```
  "scripts": {
...
```
Passons maintenant au lancement de notre page :
